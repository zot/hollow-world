      ...
      (this.hash = toHex(
        new Int8Array(
          await crypto.subtle.digest(
            'sha-256',
            protocol.utfEncoder.encode(this.text)
          )
        )
      ))
    )
...
class SpecProxyHandler {
  get(obj: any, prop: string | number | symbol) {
  ...
  set(obj: any, prop: string | number | symbol, value: any) {
    if (typeof prop === 'number' || typeof prop === 'symbol') {
      throw new Error('You can only set string properties on things')
    }
    if (
      prop === 'associations' ||
      prop === 'associationThings' ||
      prop === 'prototype' ||
      prop === 'id' ||
      prop === 'assoc' ||
      prop === 'assocId' ||
...
class AssociationIdAccessor {
    ...
    return new Proxy(this, {
      ...
      set(obj, prop: string | number | symbol, value) {
        if (
          ...
          for (const ref of obj.refs(prop)) {
  ...
  has(prop: string, tid?: thingId | Thing) {
    ...
    return (
      this.thing._associations.findIndex(
        ([ap, aid]) => ap === prop && aid === id
      ) !== -1
    )
  }
  get(prop: string): any {
  ...
  add(prop: string, tid: thingId | Thing) {
    this.set(prop, tid, true)
  }
  refs(prop?: string): Thing[] {
    return prop
      ? this.thing.world.getAssociated(prop, this.thing)
  ...
  changedAssociations() {
    this.thing._associationThings = Array.from(
      new Set(this.thing._associations.map(([, v]) => v))
    ) as thingId[]
  }
...
class SpecAssociationAccessor extends AssociationIdAccessor {
  ...
  refsProxy() {
    ...
    return new Proxy(this, {
      get(obj, prop: string | number | symbol) {
        if (typeof prop === 'string') {
          const result = obj.refs(prop).map((t: any) => t.specProxy)
          ;(result as any)._thing = result // TODO transition
          return result
        } else throw new Error(`Illegal refs property: ${String(prop)}`)
      },
...
export class Thing {
  ...
  set fullName(n: string) {
    ...
    if (
      article &&
      n.substring(0, article.length).toLowerCase() === article.toLowerCase()
    ) {
  ...
  get contentsFormat() {
    return this._contentsFormat
  }
  set contentsFormat(f: string) {
    this._contentsFormat = f
  }
  get examineFormat() {
    return this._examineFormat
  }
  set examineFormat(f: string) {
    this._examineFormat = f
  }
  get linkFormat() {
  ...
  isIn(tid: thingId | Thing) {
    return this.assocId.location === getId(tid)
  }
  formatName() {
  ...
  find(
    condition: string | ((thing: Thing) => boolean),
    exclude = new Set<Thing>()
  ) {
...
${realCode};
  ...
  setMethod(prop: any, args: string, code: string) {
    try {
      const method = this.thingEval(args, code)
      method._code = JSON.stringify([args, code])
      ;(this as any)[prop] = method
    } catch (err) {
      ;(this as any)[prop] = { _code: JSON.stringify([args, code]) }
      console.error(
        `Error loading method
@method %${this.id} ${prop} ${args} ${code}`,
        err
      )
    }
  }
  ...
  spec(): any {
    ...
    for (const prop of Object.keys(this)) {
      if (prop[0] === '_') {
        spec[prop.substring(1)] = (this as any)[prop]
      } else if (prop[0] === '!') {
        spec[prop] = (this as any)[prop]._code
      }
    }
    return spec
  }
    ...
    for (const prop of Object.keys(spec)) {
      if (prop[0] === '!') {
        ...
        if (Array.isArray(spec[prop])) {
          // rewrite arrays to strings for faster isDirty
        ...
        ;(this as any)['_' + prop] = Array.isArray(spec[prop])
          ? spec[prop].slice()
          : spec[prop]
      }
    }
...
export class World {
  ...
  async checkDbs(
    ...
    return new Promise((succeed, fail) => {
      ...
      req.onupgradeneeded = () => {
        ...
        const db = txn.db
        const userStore = db.objectStoreNames.contains(this.users)
          ? txn.objectStore(this.users)
          : db.createObjectStore(this.users, { keyPath: 'name' })
        const thingStore = db.objectStoreNames.contains(this.storeName)
  ...
  async useInfo(info: any) {
    ...
    this.roomProto =
      this.getThing(info.roomProto) || (await this.findPrototype('room'))!
    this.linkProto =
      this.getThing(info.linkProto) || (await this.findPrototype('link'))!
    this.generatorProto =
      this.getThing(info.generatorProto) ||
      (await this.findPrototype('generatorProto'))!
    this.containerProto =
  ...
  async initStdPrototypes() {
    return this.doTransaction(async () => {
      ...
      ensureProps(thingProto, {
        ...
        _enterFailFormat:
          "$forme You can't go into $event.destination $forothers $event.actor tries to go into %event.destination but can't",
        _exitFailFormat:
          "$forme You can't leave $event.origin $forothers $event.actor tries to leave %event.origin but can't",
        _closed: true,
        _priority: 0,
      })
      del(thingProto, 'go', 'get')
      thingProto.setMethod(
...
`
      ...
      ensureProps(linkProto, {
        ...
        _linkFailFormat:
          "$forme You don't have the key $forothers $Event.thing tries to go $event.direction to $event.destination but doesn't have the key",
      })
...
`
      ...
      if ((linkProto as any)['!react_newgo'])
        (linkProto as any)['!react_go'] = (linkProto as any)['!react_newgo']
...
@fail $0 "$forme You can't pick up $this! How is that even possible? $forothers $Arg tries pick up $this, whatever that means..." me
      ...
      ;(personProto as any)._event_get_thing =
        `@fail $0 "$forme You can't pick up $this! $forothers $event.actor tries to pick up $this but can't"`
...
`
  ...
  async addExtension(ext: Extension) {
    if (!this.db().objectStoreNames.contains(this.extensionsName)) {
      await new Promise((succeed, fail) => {
        ...
        req.onerror = fail
        req.onupgradeneeded = () => {
          const store = req.transaction!.db.createObjectStore(
            this.extensionsName,
            { autoIncrement: true }
          )
  ...
  async evalExtension(ext: Extension) {
    ...
    return new Promise((succeed, _fail) => {
      ...
      script.textContent = appendScriptText(
        ext.text,
        `window.textcraft.Model.registerExtension(${ext.id}, onStarted, onLoggedIn)`
      )
  ...
  getUserForThing(ti: thingId | Thing) {
    const id = idFor(ti)
    const user = id && this.userIndex.get(id)
  ...
  upgrade(fromVersion: number, spec: any) {
    if (!fromVersion) {
      ...
      if (spec.id === 'info') {
        ...
        delete spec.keys
        spec.associationThings = Array.from(
          new Set(spec.associations.map(([, v]: any) => v))
        )
      }
    }
  ...
  specAssociation(spec: any, prop: string, value = spec[prop], m2m = false) {
    if (!m2m) {
      spec.associations = spec.associations.filter(([k]: any) => k !== prop)
    } else if (
      spec.associations.find(([k, v]: any) => k === prop && v === value)
    ) {
      delete spec[prop]
      return
    }
  ...
  getInstances(proto: Thing) {
    return [...(this.prototypeIndex.get(proto.id) || [])].map((t) =>
      this.getThing(t)
    )
  }
  async toast(toasted: Set<Thing>) {
    return this.doTransaction(async (things) => {
      ...
      for (const thing of toasted) {
        if (protos.has(thing))
          throw new Error(
            `Attempt to toast prototype ${thing.id}, change references first`
          )
      }
      for (const thing of toasted) {
        const spec = thing.spec()
  ...
  copyConnected(
    connected: Set<Thing>,
    originals: Map<number, Thing>,
    copies: Map<number, Thing>
  ) {
    ...
    for (const [id, cpy] of copies as Map<number, any>) {
      const original = originals.get(id) as any
      for (const prop of Object.keys(original)) {
        if (prop === '_associations' || prop === '_associationThings') {
          continue
        } else if (copies.has(original[prop])) {
          // probably an id
          cpy[prop] = copies.get(original[prop])?.id
        } else if (original[prop] instanceof Set) {
          cpy[prop] = new Set(
            [...original[prop]].map((t) => copies.get(t) || t)
          )
  ...
  getAssociated(prop: string, thing: thingId | Thing): Thing[] {
    ...
    const things = []
    const ids = id !== undefined && this.associationIndex.get(prop)?.get(id)
  ...
  getAllAssociated(thing: thingId | Thing): Thing[] {
    const id = getId(thing)
    const things = []
    const ids = id !== undefined && this.basicAssociationIndex.get(id)
  ...
  async getOthers(thing: thingId | Thing): Promise<Thing[]> {
    ...
    return this.stamps(
      await this.doTransaction(async (things) => {
        return this.cacheThings(
          await promiseFor(
            things.index(otherLinkIndex).getAll(IDBKeyRange.only(id))
          )
        )
      })
    )
  ...
  async copyWorld(newName: string) {
    ...
    const txn = this.db().transaction(
      [this.storeName, newWorld.storeName, this.users, newWorld.users],
      'readwrite'
    )
  ...
  propertyProximity(obj: any, prop: string) {
    ...
    return found
      ? count
      : this.thingProto.hasOwnProperty(prop)
        ? count + 1
        : 1000
  }
}
...
export class MudStorage {
  ...
  upgrade(then: (arg: Event) => void): IDBOpenDBRequest {
  ...
  async strippedBlobForWorld(name: string) {
    ...
    return new Promise(async (succeed, _fail) => {
      ...
      if (dbs.length === 2) {
        result.extensions = await jsonObjectsForDb(
          txn.objectStore(extensionDbName(name))
        )
      }
  ...
  async uploadWorld(world: any, failSilently = false) {
    const w = await (world.users
      ? this.uploadFullWorld(world, failSilently)
      : this.uploadStrippedWorld(world, failSilently, world))

    if (w) this.closeWorld(w)
    return w
  }
  async uploadFullWorld(worldAndUsers: any, failSilently: boolean) {
    const users = worldAndUsers.users
    const objects = worldAndUsers.objects
    const info = objects.find((i: any) => i.id === 'info')
    if (this.hasWorld(info.name)) {
      if (failSilently) return info.name
      alert(
        `There is already a world named ${info.name}, you must rename it or delete it to upload this`
      )
      return null
    }
...
export function blobForYamlObject(object: any) {
  return new Blob(
    [jsyaml.dump(object, { noCompatMode: true, flowLevel: 3, sortKeys: true })],
    { type: 'text/yaml' }
  )
}
export function blobForJsonObjects(objects: any) {
  return new Blob(objects, { type: 'application/json' })
}

export async function blobForDb(objectStore: IDBObjectStore) {
  return blobForJsonObjects(await jsonObjectsForDb(objectStore))
}

export function jsonObjectsForDb(
...
function doAll(
  store: IDBObjectStore | IDBIndex,
  consumer: (arg: any, cursor: IDBCursorWithValue) => Promise<any>,
  range?: IDBKeyRange
) {
  return new Promise((succeed, fail) => {
...
async function copyAll(srcStore: IDBObjectStore, dstStore: IDBObjectStore) {
...
export function findSimpleName(str: string): [string | undefined, string] {
  let words: string[]
...
export function registerExtension(
  id: number,
  onStarted: (world: World, con: MudConnection) => void,
  onLoggedIn: (user: any, thing: Thing) => void
) {
